...
const Profile = () => {
  ...
  const [displayName, setDisplayName] = useState("Navigator User");
              ...
              <div key={group.label}>
                ...
                <div className="bg-card rounded-xl border border-border divide-y divide-border">
                  {group.items.map((item) => (
                    <button key={item.label} className={`w-full flex items-center gap-3 px-4 py-3 text-left ${(item as any).danger ? "text-destructive" : "text-foreground"}`}>

      ...
      <div className="w-full h-full rounded-3xl border border-border bg-card overflow-hidden shadow-lg">
       <motion.div style={{ opacity: nabOpacity }} className="absolute inset-0 bg-success/20 z-20 flex items-center justify-center pointer-events-none rounded-3xl">
        ...
        <div className="h-[55%] bg-secondary flex items-center justify-center relative">
          ...
          <span className={`absolute top-3 left-3 px-2 py-0.5 rounded-full text-[10px] font-bold ${tagColors[item.tag] || "bg-secondary text-secondary-foreground"}`}>{item.tag}</span>
          <span className="absolute top-3 right-3 text-xs text-muted-foreground font-medium bg-background/80 px-2 py-0.5 rounded-full">{item.category}</span>
          {item.hot && <span className="absolute bottom-3 right-3 text-xs bg-destructive text-destructive-foreground px-2 py-0.5 rounded-full font-bold flex items-center gap-1">🔥 {item.left} left</span>}
        </div>
        ...
        <div className="h-[45%] p-4 flex flex-col justify-between">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <div className="flex items-center gap-0.5">{[1, 2, 3, 4, 5].map((s) => <Star key={s} className={`w-3 h-3 ${s <= Math.round(item.score / 20) ? "fill-primary text-primary" : "text-muted"}`} />)}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-background pb-20">
      <div className="sticky top-0 z-40 bg-background/80 backdrop-blur-2xl border-b border-border px-4 py-3">
        <div className="flex items-center justify-between max-w-lg mx-auto">
          ...
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="icon" onClick={() => navigate("/dream-buys")}><Zap className="w-5 h-5 text-primary" /></Button>
            <Button variant="ghost" size="icon" className="relative" onClick={() => navigate("/notifications")}><Bell className="w-5 h-5" /><span className="absolute top-1 right-1 w-2 h-2 bg-primary rounded-full" /></Button>
            <Button variant="ghost" size="icon" className="relative"><Bookmark className="w-5 h-5" />{saved.length > 0 && <span className="absolute -top-0.5 -right-0.5 text-[10px] bg-primary text-primary-foreground rounded-full w-4 h-4 flex items-center justify-center font-bold">{saved.length}</span>}</Button>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-3 overflow-x-auto">
        <div className="flex gap-2 max-w-lg mx-auto">
          {categories.map((cat) => (
            <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all ${activeCategory === cat ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}>{cat}</button>
          ))}
        </div>
      </div>

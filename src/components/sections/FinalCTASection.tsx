        ...
        <div className="relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="font-heading font-bold text-foreground mb-6" style={{ fontSize: "clamp(2rem, 4vw, 3.5rem)" }}>
              Stop overpaying.<br /><span className="gradient-text">Start nabbing.</span>
            </h2>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <Button size="lg" className="rounded-full px-10 font-semibold text-base gap-2 shimmer-btn shadow-[0_0_40px_-5px_hsl(var(--coral)/0.5)]" onClick={() => navigate("/signup")}>
                Get Started Free <ArrowRight className="w-4 h-4" />
              </Button>
              <Button size="lg" variant="outline" className="rounded-full px-8 font-semibold text-base border-border hover:bg-secondary" onClick={() => document.getElementById("demo")?.scrollIntoView({ behavior: "smooth" })}>
                Try the Demo
              </Button>
            </div>

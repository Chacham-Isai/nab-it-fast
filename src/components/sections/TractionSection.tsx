  ...
  return (
    <SectionWrapper id="traction">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {traction.map((item, i) => (
          <motion.div
            ...
            className="glass-card gradient-border p-6 group relative overflow-hidden"
          >
      ...
      <motion.div 
        ...
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        className="glass-card p-4 text-center flex items-center justify-center gap-2"
      >
        <TrendingUp className="w-4 h-4 text-success" />
        <p className="text-xs text-muted-foreground">
          All metrics as of Q1 2026 · Month-over-month growth: <span className="text-success font-semibold">34% users</span> · <span className="text-success font-semibold">28% items</span> · <span className="text-success font-semibold">41% savings</span>
        </p>
      </motion.div>
    </SectionWrapper>
  );
